...
//    private static final int MSG_CHECK_BLOCK = 0x0003;
    ...
    private void pauseMusic() {
        MusicPlayService service = ((SourceGet) getApplication()).getMusicService();
        if (service != null) {
            if (service.isPlaying()) {
                ((SourceGet) getApplication()).setMusicPlayed(true);
                service.pause();
            }
        }
    }
    ...
    private void resumeMusic() {
        MusicPlayService service = ((SourceGet) getApplication()).getMusicService();
        if (service != null && ((SourceGet) getApplication()).isMusicPlayed()) {
            service.start();
        }
    }
